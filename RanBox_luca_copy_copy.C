...
void X2Pmia(Double_t *x, Double_t *p)
{
  for (Int_t i = 0; i < gNVariables; i++)
  {
    ...
    {
      p[i] += (x[j] - gMeanValues[j]) * gEigenVectors[j * gNVariables + i] / gSigmaValues[j];
    }
  }
}
...
static bool plots = false;           // whether to plot distributions
...
double Zscore_Poisson(double n, double mu)
{
  ...
  if (p < 1.E-310)
    return 37.7; // for larger significances it breaks down
...
{
  if (Non == 0 || Noff == 0 || Non == Noff / tau || tau == 0)
    return 0;
  if (Non > 0 && Noff == 0)
    return 0.;
  int Ntot = Non + Noff;
...
double ZPLsyst(int Non, int Ntot, double volume, double syst)
{
  ...
  if ((double)Non < Noff / tau)
    z = -z;
  if ((double)Non < Noff / tauhig)
    zhig = -zhig;
  if ((double)Non < Noff / taulow)
    zlow = -zlow;
  if (z > zhig)
    z = zhig;
  if (z > zlow)
...
{
  if (Non == 0 || SigboxVolume == 0 || secondSBVolume == 0)
    return 0;
...
void determineSB(double Smi[maxNvar], double Sma[maxNvar], double Bmi[maxNvar], double Bma[maxNvar], int Nvar)
{
  ...
  for (int i = 0; i < Nvar; i++)
  {
    ...
    if (AvailableRatio[i] < minratio)
      minratio = AvailableRatio[i];
  }
  ...
  for (int i = 0; i < Nvar; i++)
  {
    if (AvailableRatio[ind[i]] == 0)
      continue; // can't use this dimension
    NeededRatioPerVar = pow(2. / CurrentFactor, 1. / (Nvar - i)) - 1.;
    if (AvailableRatio[ind[i]] < NeededRatioPerVar)
    { // use all the space available for this var
      Smi[ind[i]] = 0.;
      Sma[ind[i]] = 1.;
      CurrentFactor = CurrentFactor * (1. + AvailableRatio[ind[i]]);
      if (i < Nvar - 1)
        NeededRatioPerVar = pow(2. / CurrentFactor, Nvar - i - 1) - 1.; // rescaled needed ratio for the others
    }
    else
    { // We can evenly share the volume in the remaining coordinates
      double distmin = Bmi[ind[i]];
      double deltax = Bma[ind[i]] - Bmi[ind[i]];
      if (distmin > 1. - Bma[ind[i]])
      { // Upper boundary is closest
        distmin = 1. - Bma[ind[i]];
...
void determinesecondSB(double Smi[maxNvar], double Sma[maxNvar], double Bmi[maxNvar], double Bma[maxNvar], int Nvar)
{
  ...
  for (int i = 0; i < Nvar; i++)
  {
    ...
    { // use all the space available for this var
      ...
      CurrentFactor = CurrentFactor * (1. + AvailableRatio[ind[i]]);
      if (i < Nvar - 1)
        NeededRatioPerVar = pow(3. / 2. / CurrentFactor, Nvar - i - 1) - 1.; // rescaled needed ratio for the others
    }
    else
    { // We can evenly share the volume in the remaining coordinates
      double distmin = Bmi[ind[i]];
...
double check_inputs(int Nvar, int Nsignal, int Nbackground, int NAD, double &maxHalfMuRange, int &maxJumps, int Algorithm,
                    int Gaussian_dims, bool mock, int NseedTrials, int Ntrials, bool fixed_gaussians, int NH0,
                    bool force_gaussians, int &speedup, int dataset)
{

  // Check if input parameters and other user choices are consistent
  // ---------------------------------------------------------------
  if (pow(0.5, Nvar) * (Nsignal + Nbackground) < 1)
    cout << "Warning - too few events for this Nvar" << endl;
  ...
  if (maxHalfMuRange >= 0.5)
    maxHalfMuRange = 0.499;

  if (Algorithm != 0)
    maxJumps = 0; // It is only useful for random seeding
  ...
  if (NH0 > 1)
    plots = false;
  if (speedup < 1)
    speedup = 1; // factor to speed up algorithms of clustering, set >1 (e.g. 4 or 8) for quick checks

  if (Nbackground + Nsignal <= 100)
    return -1;                                             // Avoid too small datasets;
...
void setup_strings(string &rootfile, string &asciifile, string &summaryfile, string &zplfile, bool mock, int id,
                   int Ntrials, int Nsignal, int Nbackground, int Algorithm, int speedup, int fixed_gaussians,
                   bool narrow_gaussians, bool force_gaussians, int Gaussian_dims, int NH0, int dataset,
                   bool useZPL, bool useSB)
{
  ...
  if (mock)
  {
    sstr << "/RB22_mock_" << id << "_Ntr" << Ntrials << "_NS" << Nsignal << "_NB" << Nbackground << "_A" << Algorithm;
    if (Algorithm > 0)
      sstr << "_SP" << speedup;
    if (fixed_gaussians)
    {
  ...
  if (NH0 > 1)
    zplfile = asciiPath + "/zplRB22Iter.asc";
}
                  ...
                  double maxHalfMuRange)
{
  ...
  if (dataset == 1)
    results << "miniBooNE data " << endl;
  if (dataset == 2)
    results << "Credit card frauds" << endl;
  if (dataset == 3)
    results << "EEG data" << endl;
  if (dataset == 4)
    results << "MOCK data" << endl;
  results << "  Nsignal      = " << Nsignal << endl;
  results << "  Nbackground  = " << Nbackground << endl;
  results << "  NAD          = " << NAD << endl;
  results << "  PCA          = " << PCA << endl;
  if (PCA)
    results << "  NSEL         = " << NSEL << endl;
...
void control_histos(TH1F *&Zvalue_in, TH1F *&Zvalue_fi, TH1F *&NGDsteps, int maxGDLoops, TH1F *&Ninbox_in,
                    ...
                    TH2F *&Drift, TH2F *&Ninbox_in_vs_fi, TProfile *&ZvsOrder)
{
...
// ------------------------------
void check_Ntrials(bool &doall, int NSEL, int Nvar, int &Ntrials, int maxNtr, int maxNvar, int (&Ivar)[50])
{ // 50 change
  ...
  if (Ntrials > comb / 3)
  {
    ...
    {
      Ivar[k] = k;
    }; // if we are doing the combinatorial, we need to init
    Ntrials = comb;
  }
...
void RanBox(int dataset = 0, int Ntrials = 1000, int Nsignal = 200, int Nbackground = 9800, int Algorithm = 5, bool useZPL = false,
            bool useSB = true, bool PCA = false, int Nvar = 8, int Nremoved = 0, int speedup = 1, int NH0 = 1, int c_ZPL_R2_SvsBKG = 2, bool one_off = false)
{
  ...
  //bool useSB=true; bool PCA=false; int Nvar=2; int Nremoved=0; int speedup=1; int NH0=1; bool one_off = false;  int c_ZPL_R2_SvsBKG=2;
  // Header Printout
  // ------------------------------------------------------------------------------
  header();
  ...
  TH1 *h1 = new TH1D("h3", "ULTS1", 100, 0.0, 2000);
  TH1 *h2 = new TH1D("h4", "ULTS2", 100, 0.0, 2000);
  ...
  // ------------------------------------------------------------------------------
  double FlatFrac = check_inputs(Nvar, Nsignal, Nbackground, NAD, maxHalfMuRange, maxJumps, Algorithm,
                                 Gaussian_dims, mock, NseedTrials, Ntrials, fixed_gaussians, NH0, force_gaussians, speedup, dataset);
  if (FlatFrac < 0)
    return;
  ...
  ofstream summary;
  ofstream zpl;
  setup_strings(rootfile, asciifile, summaryfile, zplfile, mock, id, Ntrials, Nsignal, Nbackground, Algorithm, speedup,
                fixed_gaussians, narrow_gaussians, force_gaussians, Gaussian_dims, NH0, dataset, useZPL, useSB);
  ...
  */
  ...
  // Set up ascii output
  // ------------------------------------------------------------------------------
  ascii_output(results, summary, zpl, rootfile, asciifile, summaryfile, zplfile, mock, id, Ntrials, Nsignal, Nbackground,
               Algorithm, speedup, fixed_gaussians, narrow_gaussians, force_gaussians, Gaussian_dims, NH0, dataset,
  ...
  control_histos(Zvalue_in, Zvalue_fi, NGDsteps, maxGDLoops, Ninbox_in, Ninbox_fi, InitialDensity, InitialVolume, NClomax,
                 ZH0, Bounds_in, Bounds_fi, Drift, Ninbox_in_vs_fi, ZvsOrder);
  ...
  double secondsidemin[maxNvar];
  double secondsidemax[maxNvar];
  ...
  cout << "Nsignal:" << Nsignal << " "
       << "Nbackground:" << Nbackground << endl;
  cout << "Nsiginfile:"
       << "  " << Nsiginfile << "  "
       << "Nbgrinfile" << Nbgrinfile << endl;
  ...
  for (int i = 0; i < goodevents; i++)
  {
    for (int dim = 0; dim < NAD; dim++)
    {
      if (feature[dim][i] < X0min[dim])
        X0min[dim] = feature[dim][i];
      if (feature[dim][i] > X0max[dim])
        X0max[dim] = feature[dim][i];
    }
  }
  for (int i = 0; i < goodevents; i++)
  {
    ...
    for (int Niter = 1; Niter < 13; Niter++)
    {
      ...
      for (int dim = 0; dim < NAD; dim++)
      {
        ...
        for (int i = 0; i < goodevents; i++)
        {
          ...
          if (ibin == pow(2, Niter))
            ibin -= 1; // avoid boundary problems
          c[ibin]++;
        }
        for (int n = 1; n < pow(2, Niter) - 1; n++)
        {
          if (c[n] == 0)
          { // bin is empty, need to compactify data support
            for (int i = 0; i < goodevents; i++)
            {
              if (feature[dim][i] > binwidth * n)
                feature[dim][i] -= binwidth * n;
            }
          }
        }
      }
  ...
  if (PCA)
  {
    ...
    for (int i = 0; i < goodevents; i++)
    {
      ...
      for (int dim = 0; dim < ND; dim++)
      {
        if (dataX[dim] < OXmin[dim])
          OXmin[dim] = dataX[dim];
        if (dataX[dim] > OXmax[dim])
          OXmax[dim] = dataX[dim];
        if (dataP[dim] < Xmin[dim])
          Xmin[dim] = dataP[dim];
        if (dataP[dim] > Xmax[dim])
          Xmax[dim] = dataP[dim];
        ...
        do
        {
          ...
          for (int ivoided = 0; ivoided < Nvoided; ivoided++)
          {
            if (voidedind[ivoided] == varind1[ihighest] ||
                voidedind[ivoided] == varind2[ihighest])
              voidthis = true;
          }
          if (voidthis)
            ihighest++;
        } while (voidthis && ihighest < rhoindex);
        if (ihighest == rhoindex)
  ...
  for (int trial = 0; trial < Ntrials; trial++)
  {
    ...
    { // scan all possible combinations of Nvar in NSEL
      if (trial > 0)
      { // // the first trial has ivar already set
        ...
        else
        {
          ...
          {
            if (Ivar[k] == NSEL - 1 - (Nvar - 1 - k))
            { // it's at the end
              Ivar[k - 1]++;
              for (int kk = k; kk < Nvar; kk++)
              {
                Ivar[kk] = Ivar[kk - 1] + 1;
              };
              onemore = false;
    ...
    else if (NseedTrials == 1)
    {
      ...
      for (int k = 0; k < Nvar; k++)
      {
        if (mock && PCA == false && Nsignal > 0 && force_gaussians == true)
        { // Pick subspace with gaussians
          Ivar[k] = (int)gRandom->Uniform(0., Gaussian_dims - smallnumber);
        }
        else
        { // Pick subspace at random
    ...
    for (int gd = 0; gd < maxGDLoops; gd++)
    {
      ...
      for (int k = 0; k < Nvar; k++)
      {
        ...
        else
        {
          ...
          if (VolumeMod[k][0] > maxBoxVolume)
            okmove[k][0] = false;
        }

        Bmin[k][1] = epsilon * (int)(InvEpsilon * (Blockmin[k] + lambda[k][1]));
        Bmax[k][1] = Blockmax[k];
        if (Bmin[k][1] > Bmax[k][1] - epsilon)
          Bmin[k][1] = Bmax[k][1] - epsilon;
        VolumeMod[k][1] = multiplier * (Bmax[k][1] - Bmin[k][1]);
        if (VolumeMod[k][1] > maxBoxVolume)
          okmove[k][1] = false;

        Bmax[k][2] = epsilon * (int)(InvEpsilon * (Blockmax[k] - lambda[k][2]));
        Bmin[k][2] = Blockmin[k];
        if (Bmax[k][2] < Bmin[k][2] + epsilon)
          Bmax[k][2] = Bmin[k][2] + epsilon;
        VolumeMod[k][2] = multiplier * (Bmax[k][2] - Bmin[k][2]);
        if (VolumeMod[k][2] > maxBoxVolume)
          okmove[k][2] = false;
        ...
        else
        {
          ...
          if (Bmin[k][4] < 0.)
            Bmin[k][4] = 0.;
          if (Bmax[k][4] <= Bmin[k][4])
            Bmax[k][4] = Bmin[k][4] + epsilon;
          VolumeMod[k][4] = multiplier * (Bmax[k][4] - Bmin[k][4]);
          if (VolumeMod[k][4] > maxBoxVolume)
            okmove[k][4] = false;
        }
        ...
        else
        {
          ...
          if (Bmax[k][5] > 1.)
            Bmax[k][5] = 1.;
          if (Bmin[k][5] >= Bmax[k][5])
            Bmin[k][5] = Bmax[k][5] - epsilon;
          VolumeMod[k][5] = multiplier * (Bmax[k][5] - Bmin[k][5]);
          if (VolumeMod[k][5] > maxBoxVolume)
            okmove[k][5] = false;
        }
      ...
      for (int i = 0; i < goodevents; i++)
      {
        ...
        for (int k = 0; k < Nvar; k++)
        {
          if (donedir[k])
            continue;
          if (in_ == Nvar || (!in[k] && in_ == Nvar - 1))
          { // either the event is in original box
            ...
            for (int m = 0; m < 7; m++)
            {
              ...
              {
                if (in_grad[k][m])
                  Nin_grad[k][m]++; // event is in modified box along var k
              }
            }
          }
        }
      ...
      if (Nin < 1)
      { // The box is too small, or the seeding went bad for some reason
      ...
      if (gd == 0)
      {
        ...
        // ADDED
        double sup = Sup(Nside);
        double eff = ((double) Nin) / goodevents; 
        double ults_start = UpperLimitTS(sup, eff);
        h2->Fill(ults_start);
        cout << "Starting from TS = " << ults_start << '\n';
      }
      ...
      if (c_ZPL_R2_SvsBKG == 0)
      {
        ...
        { // When sidebands have no events, the signal box volume is used
      ...
      else if (c_ZPL_R2_SvsBKG == 2)
      { // For the moment we put the r ratio = 1; we do so by putting twice the same volume #MODIFICA NEL COMPUTO DI SVSBKG 6/10
        /*if (useSB && Nside > 0)
        {
          Nexp = Nside;
      ...
      dirgrad = 0;
      if (debug)
        cout << "Zstart= " << Zval_start << "Ni,e=" << Nin << " " << Nexp << endl;
      for (int k = 0; k < Nvar; k++)
      {
        if (donedir[k])
          continue;
        for (int m = 0; m < 7; m++)
        {
          if (okmove[k][m])
          {
            ...
            { // For the moment we put the r ratio = 1; we do so by putting twice the same volume
              /*
              if (useSB && Nside_grad[k][m] > 0 && VolumeMod[k][m] < 0.5)
              {
                Nexp = Nside_grad[k][m];
              ...
              if (useSB && Nside_grad[k][m] > 0 && VolumeMod[k][m] < 0.5)
              {
                Nexp = Nside_grad[k][m];
                double sup = Sup(Nexp);
                double eff = ((double) Nin_grad[k][m]) / goodevents;
                Zval[m] = UpperLimitTS(sup, eff);
              }
              else
              {
                // Alex: do I need this interpolation here?..
                Nexp = VolumeMod[k][m] * goodevents;
                double sup = Sup(Nexp);
                double eff = ((double) Nin_grad[k][m]) / goodevents;
                Zval[m] = UpperLimitTS(sup, eff);
            ...
            // ------------------------------------
            if (Zval[m] > Zvalbest + epsilon)
            { // the move must improve and be the best
              igrad = k;
              dirgrad = m;
              Zvalbest = Zval[m];
      ...
      if (dirgrad == 6)
        jumps[igrad]++;
      if (jumps[igrad] >= maxJumps)
        okmove[igrad][6] = false;
      ...
      { // we can improve the Z value by moving
        ...
        if (dirgrad == 6)
        {
          for (int m = 0; m < 6; m++)
          {
            if (lambda[igrad][m] < InitialLambda / 4)
              lambda[igrad][m] = InitialLambda / 4;
          }
        }
        else
        {
          for (int m = 0; m < 6; m++)
          {
            if (m != dirgrad)
            {
              ...
              if (lambda[igrad][m] < epsilon)
                lambda[igrad][m] = epsilon;
        ...
        if (widening_factor > 1)
        {
          ...
          if (dirgrad < 6)
          {
            if (lambda[igrad][dirgrad] < epsilon)
              lambda[igrad][dirgrad] = epsilon;
          }
    ...
    // ### HERE I CAN SLIDE THE Sup CALCULATIONS, because now everything in the box is collected
    sbbratio = SidebandsVolume / VolumeOrig;
    secsbbratio = SecondsidebandsVolume / VolumeOrig;
    if (Nsidebest != 0 && Ninbest != 0)
    {
      // Ratio1 = Ninbest / (Nsidebest * (SidebandsVolume - VolumeOrig) / VolumeOrig);
      double sup = Sup(Nsidebest);
      double eff = ((double) Nin_best) / goodevents;
      double ults = UpperLimitTS( sup, eff );
      h1->Fill(ults);
      cout << "trial " << trial << ", best ults = " << ults << endl;
    }
    /*
 	if (N2sb !=0 && Ninbest !=0) { 
	 Ratio2 = Ninbest/(N2sb*(SecondsidebandsVolume-2.*VolumeOrig)/VolumeOrig);
	 h2->Fill(Ratio2);
	 cout << "iteration " << trial << ", R2 = " << Ratio2 << endl;
	}
 	if (N2sb !=0 && Ninbest !=0) { 
	 Ratio3 = Ninbest/N2sb;			// we expect to see something centered at 1
	 h3->Fill(Ratio3);
	 cout << "iteration " << trial << ", R3 = " << Ratio3 << endl;
	}
	*/
  ...
  if (Nsignal > 0)
  {
    ...
    if (NseedTrials > 1)
      cout << "  This time AverSF=" << (double)NSignalInBox / Nsignal << "  Aver_1s= " << ndimin << endl;
...
*/
  ...
  if (IH0 == 0 && mock)
    summary << " FixG=" << fixed_gaussians << " NarrG=" << narrow_gaussians << " forceG=" << force_gaussians << " maxR=" << maxRho << " maxHMR=" << maxHalfMuRange;
  if (IH0 == 0)
    summary << endl;
  if (IH0 == 0)
    summary << "id  Ntr  NS  NB  Alg maxZ useSB Nvar spup Z  Nin Nex Ns Vol BoxSF SBg BoxVars" << endl;
  if (IH0 == 0)
    summary << "---------------------------------------------------------------------------------------" << endl;
  ...
  TCanvas *c1 = new TCanvas("c111", "canvas111", 600, 400);
  c1->cd();
