...
// by first transforming the feature space with principal component
// analysis, and then flattening the marginals, then searching the
// data for a multi-D box of dimensionality smaller than the original
// one where there are more data events than expected.
// The expectation is computed by considering the box volume, or by
...
//
// T. Dorigo, March - June 2019
//
// To compile: g++ -g `root-config --libs --cflags` RanBox.cpp -o RanBox
//
...
//
//                Original creation - Tommaso Dorigo, 2019-2021
// ------------------------------------------------------------------------------------------------------

void RanBox(int dataset = 0, int Ntrials = 1000, int Nsignal = 200, int Nbackground = 9800,
			int Algorithm = 5, bool useZPL = false,
			bool useSB = true, bool PCA = false, int Nvar = 8,
			int Nremoved = 0, int speedup = 1, int NH0 = 1)
{

	// dataset: type of dataset used in search:
	// 0=HEPMASS; 1=miniBooNE; 2=fraud data; 3=EEG data; 4=synthetic dataset (mock)
	...
	// NH0: if >1, the search is done NH0 times to populate a graph of the test statistic under the null
	...
	if (NAD < Nvar)
	{
		...
		return; // (to avoid infinite loop in choosing vars)
	}
	if (Nvar < 2)
	{
	...
	if (Nbackground + Nsignal <= 100)
		return;												 // Avoid too small datasets;
	double FlatFrac = Nbackground / (Nbackground + Nsignal); // FlatFrac: fraction of flat events in toy
															 // (the rest are multivariate normal)

	// Defaults
	// --------
	string outputPath = "RANBOX"; // "Output"; // "/lustre/cmswork/dorigo/RanBox/Output";
	string asciiPath = "RANBOX";  // Ascii"; // "/lustre/cmswork/dorigo/RanBox/Ascii";
	string zpath = "RANBOX";
	...
	if (mock)
	{
		sstr << "/RB22_mock_" << id << "_Ntr" << Ntrials
	...
	for (int IH0 = 0; IH0 < NH0; IH0++)
	{
		...
		if (mock)
		{
			...
			for (int dim = 0; dim < NAD; dim++)
			{
				...
				if (currchar > 52)
					currchar -= 53;
				for (int ch = 1; ch <= incr; ch++)
				{
					cout << progress[currchar + ch];
				};
				currchar += incr;
				for (int i = 0; i < Nmock; i++)
				{
		...
		{ // Not mock data: read real data
			...
			{ // HEPMASS dataset
				...
				{
					if (p == 5)
						maxletter2 = 11; // only files up to fk are available
					for (int q = save_q; q < maxletter2; q++)
					{ //when we are at the end of a file we go back here
						if (closed)
						{
							dataname = "RANBOX/HEPMASS/whiteson_" + alphabet[p] + alphabet[q];
							file.open(dataname);
						...
						if (!readok)
						{
							...
							file.close();
							closed = true;
							if (p == maxletter1 - 1 && q == maxletter2 - 1)
							{ // reset files if we got to the end of the list
								save_p = 0;
								save_q = 0;
								p = 0;
								q = -1; // so that it goes back to 0,0
							}
							if (save_q == maxletter2 - 1)
								save_q = 0; //reset save_q value
						}
						else
						{
			...
			{ // miniBooNE dataset
				...
				dataname = "RANBOX/MiniBooNE/miniBooNE_PID.txt";
			...
			{ // credit card data
				// Start of loop
				// -------------
				dataname = "RANBOX/Fraud/creditcard.asc";
				file.open(dataname);
				bool readok;
				std::string data;
				std::string::size_type sz;
			...
			{ // EEG data
				// Start of loop
				// -------------
				dataname = "RANBOX/EEG/EEG_eye_detection.asc";
				file.open(dataname);
				bool readok;
				std::string data;
				std::string::size_type sz;
			...
			if (dataset == 1)
				cout << "  N voided events read = " << Nvoided << endl;
			...
			else if (Algorithm == 1)
			{
				...
				for (int i = 0; i < goodevents; i += speedup)
				{
					...
					for (int ii = 0; ii < goodevents; ii += speedup)
					{
						if (ii == i)
							continue;
						if (AmClosest[ii] > maxClosest - 2)
							continue; // We cap the max associations
						double d2[maxNvar];
						for (int k = 0; k < Nvar; k++)
						{
		...
		if (Nsignal > 0)
		{
			...
			if (NseedTrials > 1)
				cout << "  This time AverSF=" << (double)NSignalInBox / Nsignal
					 << "  Aver_1s= " << ndimin << endl;
		...
		if (IH0 == 0 && mock)
			summary << " FixG=" << fixed_gaussians << " NarrG="
