...
id     = (int)did;
*/
// commented parts for .C implementation
//}
                  ...
                  bool useSB = true, bool PCA = false, int Nvar = 8, int Nremoved = 0, int speedup = 1, int NH0 = 1, int c_ZPL_R2_SvsBKG = 2, bool one_off = false)
{
  ...
  // Change preset generator: NB other versions of TRandom are flawed, id: Identifier for summary printouts
  // ------------------------------------------------------------------------------
  // double seed;       double did;         int id;			// ### commented because of redefinition error
  ...
  float **feature_signal = new float *[ND];
  float **feature_background = new float *[ND];
  int **order_ind = new int *[ND];
  for (int i = 0; i < ND; i++)
  {
    feature[i] = feature_all + (maxEvents)*i;
    feature_signal[i] = feature_all + (maxEvents)*i;
    feature_background[i] = feature_all + (maxEvents)*i;
    order_ind[i] = order_ind_all + (maxEvents)*i;
  }

  double *xtmp = new double[maxEvents];
  bool *isSignal = new bool[maxEvents];
  bool *isBackground = new bool[maxEvents];
  ...
  */
  ...
  if (!mock)
    NseedTrials = 1;
  int goodevents_signal;
  int goodevents_background;
  ...
  cout << "Nsiginfile:"
       << "  " << Nsiginfile << "  " << endl;
  int Ntot_signal = Nsignal;

  for (int i = 0; i < Ntot_signal; i++)
  {
    ...
    {
      if (i < Nsiginfile && goodevents_signal < Nsignal)
      {
        for (int dim = 0; dim < NAD; dim++)
        {
          feature_signal[dim][goodevents_signal] = f_signal[dim];
        }
        isSignal[goodevents_signal] = true;
        NSread++;
        goodevents_signal++;
      }
    }
  ...
  goodevents_background = 0;
  NSread = 0;
  NBread = 0;
  block = (Nbackground) / 50;
  double f_background[ND];
  int Nvoided_background = 0;
  ...
  cout << "Nbrinfile:"
       << "  " << Nbgrinfile << "  " << endl;
  int Ntot_background = Nbackground;

  for (int i = 0; i < Ntot_background; i++)
  {
    // Print out progress in nice format
    // ---------------------------------
    if (i % block == 0)
    ...
    float f_background[ND];
    int voidit = true; // We have to prune events with -999 on all features from file
    for (int dim = 0; dim < NAD; dim++)
    {
      events >> f_background[dim];
      //std::cout << f[dim] << std::endl;
      if (f_background[dim] != -0.999E+03)
        voidit = false;
    }
    if (voidit)
    {
    ...
    {
      if (i < Nbgrinfile && goodevents_background < Nbackground)
      {
        for (int dim = 0; dim < NAD; dim++)
        {
          feature_background[dim][goodevents_background] = f_background[dim];
        }
        isBackground[goodevents_background] = true;
        NBread++;
        goodevents_background++;
      }
    }
  } // End main for loop
  ...
  {
    X0min[dim] = bignumber;
    X0max[dim] = -bignumber; // need to check where we use these values
  }
  /*
    ...
    for (int gd = 0; gd < maxGDLoops; gd++)
    {
      ...
      for (int k = 0; k < Nvar; k++)
      {
        ...
        { // For the moment we put the r ratio = 1; we do so by putting twice the same volume
          ...
          {
            Nexp = Nside;
            Zval_start = SvsBKG(Nin, Nside, VolumeOrig, SecondsidebandsVolume);
          ...
          for (int m = 0; m < 7; m++)
          {
            if (okmove[k][m])
            {
              ...
              { // For the moment we put the r ratio = 1; we do so by putting twice the same volume
                ...
                {
                  Nexp = Nside_grad[k][m];
                  Zval[m] = SvsBKG(Nin_grad[k][m], Nside_grad[k][m], VolumeMod[k][m], VolumeMod[k][m]); // per ora evito di cambiare, vediamo come va
                }
                else
                {
                  Nexp = VolumeMod[k][m] * goodevents;
                  Zval[m] = SvsBKGnosb(Nin_grad[k][m], goodevents, VolumeMod[k][m], VolumeMod[k][m]); // il controllo su tale variabile è all'if qui sotto
                }
              }
              if (debug)
                cout << "Z[" << k << "," << m << "]=" << Zval[m]
