...
// Constants
// ---------
static const int maxNbest      = 50;         // number of best boxes stored and investigated
static const int ND            = 50;          // number of active dimensions considered by algorithm
static const int maxNvar       = ND;
static const int maxEvents     = 10000;
static const double sqrt2      = sqrt(2.);
static const int maxClosest    = 20;
static const double RegFactor  = 1.;  // should be larger than 1 if using useSB true
...
void RanBoxIter (int dataset=0, int Nbest=20, int Nsignal=200, int Nbackground=9800, 
		 int Algorithm=5, bool useZPL=false, 
		 bool useSB=true, bool PCA = false, int Nvarmax=10, 
  ...
  if (Nremoved>0 && !PCA) RemoveHighCorr = true;
  ...
  if (Nbest>maxNbest) {
    cout << "  Sorry can investigate at most " << maxNbest 
	 << " solutions, will do that." << endl;
    Nbest = maxNbest;
  }
  if (Nvarmax>12 && plots) {
  ...
  int id     = (int)did;
  if (Nbackground+Nsignal<=100) return;                // Avoid too small datasets;
  double FlatFrac = Nbackground/(Nbackground+Nsignal); // FlatFrac: fraction of flat events in toy 
                                                       // (the rest are multivariate normal)

  // Defaults
  // --------
  string outputPath = "RANBOXIter"; // "Output"; // "/lustre/cmswork/dorigo/RanBox/Output";
  string asciiPath  = "RANBOXIter"; // Ascii"; // "/lustre/cmswork/dorigo/RanBox/Ascii";
  string zpath      = "RANBOXIter";
  
  std::stringstream sstr;
  std::stringstream sstr2;
  if (mock) {
    sstr << "/RB22Iter_mock_" << id << "_maxNv" << Nvarmax 
    ...
    if (dataset==0) {
      sstr << "/RB22Iter_HEPMASS_" << id << "_maxNv" << Nvarmax 
	   << "_NS" << Nsignal << "_NB" << Nbackground << "_A" << Algorithm;
    } else if (dataset==1) {
      sstr << "/RB22Iter_miniBooNE_" << id << "_maxNv" << Nvarmax 
	   << "_NS" << Nsignal << "_NB" << Nbackground << "_A" << Algorithm;
    } else if (dataset==2) {
      sstr << "/RB22Iter_fraud_" << id << "_maxNv" << Nvarmax 
	   << "_NS" << Nsignal << "_NB" << Nbackground << "_A" << Algorithm;
    } else if (dataset==3) {
      sstr << "/RB22Iter_eeg_" << id << "_maxNv" << Nvarmax 
	   << "_NS" << Nsignal << "_NB" << Nbackground << "_A" << Algorithm;
    } else if (dataset==4) {
      sstr << "/RB22Iter_mock_" << id << "_maxNv" << Nvarmax 
	   << "_NS" << Nsignal << "_NB" << Nbackground << "_A" << Algorithm;
  ...
  string zplfile;
  if (dataset==0) {
    summaryfile = asciiPath + "/summaryRB22Iter_HEPMASS.asc";
  } else if (dataset==1) {
    summaryfile = asciiPath + "/summaryRB22Iter_miniBooNE.asc";
  } else if (dataset==2) {
    summaryfile = asciiPath + "/SummaryRB22Iter_fraud.asc";
  } else if (dataset==3) {
    summaryfile = asciiPath + "/SummaryRB22Iter_eeg.asc";
  } else if (dataset==4) {
    summaryfile = asciiPath + "/summaryRB22Iter_mock.asc";
  }
  ...
  cout << "  ------------------------------------------------------------------------------------------------" << endl;
  cout << endl;
  cout << "                                  R  A  N  B  O  X    I  T  E  R   2 2                            " << endl << endl;
  cout << "                                 --------------------------------------                           " << endl;
  cout << endl;
  ...
  results << "  --------------------------------------- " << endl;
  results << "           R A N B O X   I T E R   2 2    " << endl;
  ...
  for (int IH0=0; IH0<NH0; IH0++) {
	...
	for(int p=save_p; p<maxletter1; p++) {
	  ...
	  for(int q=save_q; q<maxletter2; q++) { //when we are at the end of a file we go back here      
	    if (closed) {
	      string dataname = "RANBOX/HEPMASS/whiteson_" + alphabet[p] + alphabet[q];
	      file.open(dataname);
	...
	if (ibest>0) {
	  ...
	  for (int ib=0; ib<ibest; ib++) {
	    ...
	    if (match==pow(2,Nvar-1)) { // both combs must be of len Nvar-1 as previously enforced, so if N-1 vars match this is done already
	      if (verbose) {cout << "Same comb, skipping: " << ib << " " << ibest << " ";
		for (int k=0; k<Nvar; k++) {
		  for (int k2=0; k2<Nvar-1; k2++) {
...
/* 		  } */
	...
	cout << "  BoxSF=" << 0.1*(int)(10*BoxSFr[i]) << "% SBG=" << sbgain;
      ...
      cout << " " << BoxNva[i]<<" - vars = ";
      if (i==0) {
	summary << 0.1*(int)(10*BoxZpl[i]) 
		<< "  " << BoxNin[i]
		<< "  " << 0.01*(int)(BoxNex[i]*100)
		<< "  " << 0.1*(int)(10*BoxSFr[i]*Nsignal/100.)
		<< "  " << 0.0001*(int)(10000*BoxVol[i]);
	if (Nsignal>0) {
